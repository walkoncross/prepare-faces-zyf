...
def main(list_fn, root_dir=None):
    save_fn = osp.splitext(list_fn)[0] + '_fixed3.txt'
    ...
    for line in fp:
        ...
        if idx != start_id:
            ...
            if len(tmp_list) > 1:
                ...
                for it in tmp_list:
                    id_name = it.split('/')[0]
                    if id_name not in id_name_cnt_dict:
                        id_name_cnt_dict[id_name] = 1
                    else:
                        id_name_cnt_dict[id_name] += 1

                prime_id_name = max(
                    id_name_cnt_dict, key=lambda k: id_name_cnt_dict[k])

                for it in tmp_list:
                    if it.startswith(prime_id_name):
                        prime_cnt += 1
                        fp_rlt.write('%s %d\n' % (it, new_idx))

                print('\t but only keep {} prime_id image file with {} new_ids\n'.format(
                    prime_cnt, new_idx + 1))
            ...
            # if the same id num and the same id name
            if name == last_name:
                # fixed_subdir does not exist
                if fixed_subdir is None:
                    fail_cnt += 1
                    print('Cannot find fixed_subdir for: ' + fn_im_0)
                    continue
                else:  # use previous fixed_subdir if it exists
                    fixed_fn = osp.join(fixed_subdir, fn_im_2)
    ...
    save_fn2 = osp.splitext(
        list_fn)[0] + ('_fixed3_%d_ids_%d_imgs_fixed.txt' % (new_idx + 1, prime_cnt))
...
if __name__ == '__main__':
    ...
    list_fn = './MS-Celeb-1M_clean_list.txt'
